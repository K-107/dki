    ...
    try:
        ...
        try: 
            with open(txt, "r", encoding='utf-8') as text:
                output_name, output_path, show_boxes, normalization = searchText(input_image, txt, img, w, h, text, class_dic, input_output_path)
...
def searchText(input_image, txt, img, w, h, text, class_dic, input_output_path):
    ...
    for num in range(len(lines)):
        ...
        if len(m_line) == 5:
            ...
            '_'.join(m_line[6:]), float(m_line[0]), float(m_line[1]), float(m_line[2]), \
                float(m_line[3]), float(m_line[4]), float(m_line[5])                
            print("label, w_txt, h_txt, x_coord, y_coord, bbox_w, bbox_h", label, w_txt, h_txt, x_coord, y_coord, bbox_w, bbox_h)       
            ...
            if label == 'None':
                ...
                print('label111111111111111: ', label)
